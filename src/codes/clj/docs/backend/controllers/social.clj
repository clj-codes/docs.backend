...
(defn upsert-author
  {:malli/schema [:=> [:cat schemas.model.social/NewAuthor schemas.types/Components] :any]}
  [new-author {:keys [database]}]
  (db.postgres/upsert-author new-author database))

(defn get-author
  {:malli/schema [:=> [:cat :string :string schemas.types/Components] schemas.model.social/Author]}
  [login source {:keys [database]}]
  (db.postgres/get-author login source database))

(defn insert-see-also
  {:malli/schema [:=> [:cat schemas.model.social/NewSeeAlso schemas.types/Components] :any]}
  [new-see-also {:keys [database]}]
  (db.postgres/insert-see-also new-see-also database))

(defn insert-example
  {:malli/schema [:=> [:cat schemas.model.social/NewExample schemas.types/Components] :any]}
  [new-example {:keys [database]}]
  (db.postgres/insert-example new-example database))

(defn update-example
  {:malli/schema [:=> [:cat schemas.model.social/UpdateExample schemas.types/Components] :any]}
  [update-example {:keys [database]}]
  (db.postgres/update-example update-example database))

(defn insert-note
  {:malli/schema [:=> [:cat schemas.model.social/NewNote schemas.types/Components] :any]}
  [new-note {:keys [database]}]
  (db.postgres/insert-note new-note database))

(defn update-note
  {:malli/schema [:=> [:cat schemas.model.social/UpdateNote schemas.types/Components] :any]}
